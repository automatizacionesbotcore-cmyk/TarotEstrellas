...
- Rama de diseno activa: `feat/ui-ux-pro-max-redesign`, subida a `origin/feat/ui-ux-pro-max-redesign`.
- Frontend: usar `pnpm`, no `npm`. El frontend declara `packageManager: pnpm@11.1.2`, usa `pnpm-lock.yaml` y `pnpm-workspace.yaml`; `package-lock.json` fue retirado.
- Build verificado: `pnpm build` pasa. Queda warning conocido de chunks grandes en Vite (`index`, `webgl`, `SalaVideoPage`).
- Pruebas recientes:
  - `pnpm test` pasa: 1 archivo, 2 tests.
  - Backend unitario pasa: `php artisan test --testsuite=Unit`, 4 tests / 8 assertions.
  - Backend completo y `Feature --stop-on-failure` quedaron en timeout local; revisar antes de merge si el PR toca backend.
- Ultimos commits relevantes de UI:
  - `50e5635 feat: completar pulido visual del admin`
  - `e1fa3dd feat: pulir responsividad del super admin`
  - `beaaa56 feat: pulir reserva y pantallas de pago`
  - `72d1397 feat: pulir paginas publicas y flujo cliente`
  - `63e38f4 feat: redisenar UI y mejorar responsividad admin`
